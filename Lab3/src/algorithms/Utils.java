...
public class Utils {
     ...
     * @return - merged sorted array
     */
    public static double[] mergeDescending(double[] a1, double[] a2){
        if(!isSortedDescending(a1) || !isSortedDescending(a2)){
            throw new RuntimeException(
                    "One of the arrays is not sorted in descending order.");
        }
        double[] result = new double[a1.length+a2.length];
        int i1 = 0;
        int i2 = 0;
    ...
    public static boolean isSortedDescending(double[] a) {
        for (int i = 0; i < a.length - 1; i++) { 
            if (a[i] < a[i + 1]) {
                return false;
            }
    ...
    public static void reverseSortArray(double[] myData) {
        int dataLength = myData.length;
        Arrays.sort(myData);
        double tmp;
        for(int i = 0 ; i < dataLength /2; i++) {
            tmp = myData[i];
            myData[i] = myData[dataLength - i - 1];
            myData[dataLength - i - 1] = tmp;
        }
    }
}
